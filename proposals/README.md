...
`if(X)` and `return(X + Y)`?  (a la Pari-GP?)  e.g.,
...
# then is not always needed if you return something
# (instead of doing e.g., `Then exit(3)`)
Result: if(X, (): 3, else(): 4)

# this is a bit of a bother with Horstmann indents.
Result: if
(   X
    (Then):
        print("X was truthy")
...
```
but i think i like how it simplifies parsing.  if we do this, we probably
should not allow naked `return` values, we should use `Function_name exit(3)`
where `Function_name` is the `Variable_case` version of the `function_case` name.
e.g.,
```
my_function(X: int, Y: int): int
    ...
    (   X + Y
        ():
            print("X + Y was truthy")
            My_function exit(123)
        else(Then):
            Then exit(5)
    )
    My_function exit(Result + 7)
```
